...
// buffersize is the  [buf]--in the struct  NALU_t
...
// buffersize is the  [n]->[buf]  --in the struct  NALU_t
...
// fp------> link to the [ tmp x264--output file ]
...
// find statcode2|3  the logic is familiar ----- 
// input buf -- detect the first 2|3 bytes
int FindStartCode2(unsigned char *Buf)
{
...
void rtpSend(char *path)
{
	...
	{	size=GetAnnexbNALU(n);   // huge bits usage 
		...
		dump(n); // detect the n---> NALU_t is NULL
		if(!start)
		{
...
void SendH264Nalu(unsigned char* m_h264Buf,int buflen)
{
    ...
    if ( buflen <= MAX_RTP_PKT_LENGTH )   //1360 : MAX_RTP_PKT_LENGTH 
    {    
    ...
    {  
        ...
        {    
            ...
            {  
				...
				*/
                sendbuf[0] = (nalHeader & 0x60)|28;   //NRI   | 28  Fragment Unit -- A ,29 B
                sendbuf[1] = (nalHeader & 0x1f); 	  //TYPE	
                if ( 0 == t ) 
                { 
                    sendbuf[1] |= 0x80; 
                } 
