...
//===================================================
QString ipaddr1 = "125.65.110.15";          //中心服务器IP/端口
int port1 = 1100;
//@0108
//QString ipaddr1 = "222.197.174.76";       //模拟服务器
//int port1 = 1100;
//=====================================================
QString ipaddr2 ;     //上位机地址
int port2 ;
    ...
    QThread(parent)
{
    ...
    Insert_Or_Change = 2;
    I_OR_C_Num = 11;
    Finger_Partition_number = 0;
    Finger_number = 0;//指纹序号

    Upper_Or_Center = FALSE;
    Receive_IP_Sucess = false;

    DATA_SEND_SUCCESS = 0x00;
  ...
  //  qDebug()<<"Upper_Port"<<port2;
    ...
    if (time_step != 1)
    {
     ...
     //  Connect_Gprs();     // AT
        ...
        {
           qDebug()<<"binding  udpSocket  failed";
        }
        else
        {
            qDebug()<< "udpSocket  bind "<<udpSocket->localPort();
            qDebug()<< "udpSocket  local IP"<< udpSocket->localAddress();
            qDebug()<< "udpSocket_Upper  bind "<<udpSocket_Upper->localPort();
            qDebug()<< "udpSocket_Upper  local IP"<< udpSocket_Upper->localAddress();
        }
...
void GprsConnect::run()
{
        ...
        qDebug()<<"in the gprs::run()";
         ...
         // @1115
         if (globalData->identity_success == 1)  //@0108 改成了 0，需要还原1
        {
             send_message_to_center();
             qDebug()<<"globalData->identity_success == 1";
         }

         else
         {
           // msleep(3000);
          //  qDebug()<<"globalData->identity_success == 0";
         }
...
//===============// data from upper===============
    if(center_receive == 0)
    {
        ...
        {
                    if(Upper_Or_Center == false)  //服务器
                    {
                            for(int i = 2;i<data.size()-2;i++)
                            {
                                //将接收的数据暂存在数组中 *去侦头帧尾* included CRC_data
                                tem_data_receive[i-2] = data.data()[i];
                             }
                    ...
                    else  //上位机数据
                    {
                        for(int i = 0;i<data.size()-2;i++)
                           {
                                //将接收的数据暂存在数组中，*去帧尾*
                                tem_data_receive[i] = data.data()[i];
                            }
...
void GprsConnect::center_data_proc( char *tem_data_receive , quint32 tem_data_receive_len)
{
    quint8 temp[2];
    char tem_data[200];
    qint32 data_len = tem_data_receive_len;// 去了帧尾，留侦头
        ...
        if ((tem_data_receive[data_len-1] == temp[1]) && (tem_data_receive[data_len-2] == temp[0]))
        {
            ...
            qDebug()<<"******frame type*******0x"<<hex<<(quint8)frame_type;
            switch (frame_type)
            {
            case 0x01://身份验证回应帧
                        {
            ...
            case 0x04://信息传输帧，为上位机下发的配置信息
            {
                ...
                case 0x06://力矩表--------no  testing
                {
                    ...
                    int moment_data_len = data_len - 24;
                    int writelen;
...
先发送指纹版本对比请求（只发一次，如果和数据库中的不一致则从?上位机?服务器下载数据）——>发送信息传输帧（只发一次)
*************************************/
void GprsConnect::send_message_to_center()
{
   // qDebug()<<"enter   GprsConnect::send_message_to_center()  ";


       //@1115*******************************
        send_mes_trans_data();  //信息传输信息发送，为开机时必须发送的  ---1  Center
        send_finger_version_inquiry();  //指纹版本请求发送，为开机时必须发送的  ---1 Center
        msleep(500);
...
//下面的判断表示指纹请求和信息传输帧已发送成功
 if((DATA_SEND_SUCCESS == 0x03) && (Config_Info_Flag == 0x00))
        {
                    if(globalData->fingerPage[0] != 0x13)//如果收到指纹打卡，就上发身份报告
                    {
                        report_identity_to_center();
                    }
         //@1115*******************************

                    if(Receive_IP_Sucess == false)
                    {      //如果没收到已经解析我IP端口的回应帧就发送数据
                        qDebug()<<"send ip to upper";
                        send_IP_Data_ToUpper();//往上位机发送信号，使得上位机可以识别本地<ip,port>
                        msleep(2000);
                    }

                     //qDebug()<<"send ip to upper  outer";
...
void GprsConnect::info_send(char *send_info_data, quint32 info_len , QString ipaddr , int port)
{
    ...
    msleep(1000);
    // udpSocket->writeDatagram((char*) send_info_data,frame_len,QHostAddress("222.197.174.69"),30000);
        //udpSocket->writeDatagram( send_info_data,frame_len,QHostAddress("125.65.110.15"),1100);
        udpSocket->writeDatagram( send_info_data,frame_len,QHostAddress(ipaddr),port);
...
int GprsConnect::Open_Serial(quint8 U_or_S,quint32 baultrate,quint8 S_num)
{
    qint32 fd;
        ...
        {      //打开串口,如果打开失败，会返回-1
                qDebug() << "Open USB port failed , S_num";

       }

          if (setComConfig( fd , baultrate , 8 , 'N' , 1 ) < 0 )
       {//打开端口错误，返回-1
            qDebug() << "Set COM failed ,S_num";
...
//for(quint8 i =0;i<4;i++)
//{
//    qDebug()<<"send time"<<hex<<Temp_Status_Data[i];
//}
...
void GprsConnect::time_synchron(char *Time_str)
{
  ...
  //  quint8 yy_char[1]={0},MM_char[1]={0},dd_char[1]={0},hh_char[1]={0},mm_char[1]={0},ss_char[1]={0};
   ...
   // qDebug()<<system_time.currentDateTime();
    ...
    system(str.toLatin1().data());  //set the kernel time

//    system(str.toLatin1().data());
    system("hwclock -w &");         //set the hardware time
...
void GprsConnect::send_mes_trans_data()
{
      ...
      //标志位第一位为基本信息发送标志位，标志位起到保证基本信息只发送一遍
        {
            qDebug()<<"************BASIC************";
            send_basic_data();
            msleep(500);
...
void GprsConnect::identity_illegal_dispose()
{
    char identy_illegal = 0x05;
    char identy_illegal_content[2] = {0x01, 0x02};

    if( (globalData->fingerPage[0] == 0x13) && ( globalData->fingerChance == 0))
        {
            report_illegal_to_center(identy_illegal, identy_illegal_content);//未登录
        }
    else if(globalData->fingerPage[0] == 0xa0)
        {
            report_illegal_to_center(identy_illegal, identy_illegal_content+1);//验证失败
            globalData->fingerPage[0] = 0x13;
        }
...
void GprsConnect::report_identity_to_center()
{
    ...
    memset(sys_driver_ID ,0 ,4);
    if(globalData->fingerPage[0] < 0x13 )
    {
...
bool GprsConnect::Connect_Gprs()
{
    ...
    if ( ( fd = openPort( DATA_TRANSFER_COM_0,1) )< 0 )//打开串口,如果打开失败，会返回-1
    {
        qDebug() << "Open USB port failed,GPRS ";
        return FALSE;
    }
    else
    {
        qDebug()<<" GPRS com  fd *****  "<<fd;
    }

    if (setComConfig( fd , 115200 , 8 , 'N' , 1 ) < 0 )  //打开端口错误，返回-1
    {
        qDebug() << "Set COM failed,GPRS";
        return FALSE;
    }
...
//            memset(buff,0,BUFFER_SIZE);
//           //  msleep(1000);
//            qDebug()<<"Input ----  CGMM        -----  command";
//            strcpy(sendData,"AT+CGMM=?\r\n");
//             write(fd,sendData,strlen(sendData));
//             memset ( buff , 0 , BUFFER_SIZE );
//             receivedLen = read ( fd , buff , 30 );
//              qDebug()<< buff;
...
//            memset(buff,0,BUFFER_SIZE);
//           //  msleep(1000);
//            qDebug()<<"Input ----    NWINFO      -----  command";
//            strcpy(sendData,"AT%NWINFO\r\n");
//             write(fd,sendData,strlen(sendData));
//             memset ( buff , 0 , BUFFER_SIZE );
//             receivedLen = read ( fd , buff , 30 );
//              qDebug()<< buff;
...
//            memset(buff,0,BUFFER_SIZE);
//           //  msleep(1000);
//            qDebug()<<"Input ----   SYSCFG       -----  command";
//            strcpy(sendData,"AT%SYSCFG=2,2,3FFFFFFF,1,2\r\n");
//             write(fd,sendData,strlen(sendData));
//             memset ( buff , 0 , BUFFER_SIZE );
//             receivedLen = read ( fd , buff , 30 );
//              qDebug()<< buff;
...
//            break;
        ...
        case CGDCONT:
            memset(buff,0,BUFFER_SIZE);
           //  msleep(1000);
            qDebug()<<"Input AT+CGDCONT command";
        ...
        case ATD:
            memset(buff,0,BUFFER_SIZE);
             qDebug()<<"Input ATDT command";
            //strcpy(sendData,"ATDT*99***1#\r\n");
            strcpy(sendData,"ATD*99#\r\n");
            memset(buff,0,BUFFER_SIZE);
           // qDebug()<< sendData;
            sleep(2);
            write(fd,sendData,strlen(sendData));
