...
GlobalData::GlobalData()
{
    ...
    // 20140830  打开COM3   配合  diag.cpp   zigbeeCfg
     //OpenSerialPort();
...
quint16 GlobalData::dataCRC16(char *pucSendBuf , quint32 usLen , quint8 *CRC)     //CRC16校验算法
{
    quint16  i, j ;
...
{
    if ( ( ZigbeeFd = openPort( DATA_TRANSFER_COM3) ) < 0 ) {      //打开串口
        qDebug() << "******Open failed  in  GlobalData::OpenSerialPort();";
        qDebug()<<"***************************************";
    }
    else
    {
        qDebug()<<"*****Open port successCOM_3   38400_8n1_  "<<ZigbeeFd;
        qDebug()<<"ZigbeeFd="<<ZigbeeFd;
       // qDebug()<<"***************************************";
    }

    if (setComConfig( ZigbeeFd , 38400 , 8 , 'N' , 1 ) < 0 ){
        qDebug() << "*****Set COM failed";
        qDebug()<<"***************************************";
    }
    else
    {
//        qDebug()<<"*****Set COM success !,38400_8n1_";
//        qDebug()<<"***************************************";
    }
}
